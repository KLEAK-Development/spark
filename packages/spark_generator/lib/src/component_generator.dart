...
class ComponentGenerator extends GeneratorForAnnotation<Component> {
  /// Getter names generated by the code generator or inherited from
  /// SparkComponent/WebComponent that must not be overwritten by user code.
  static const _reservedGetterNames = {
    'tagName',
    'observedAttributes',
    'dumpedAttributes',
    'adoptedStyleSheets',
  ...
  /// Field names generated or inherited that must not be overwritten.
  ...
  /// Method names generated or inherited that must not be overwritten.
  static const _reservedMethodNames = {
    'render',
    'build',
    'onMount',
    'syncAttributes',
  ...
  String _generateCompleteReactiveClass(
    ...
    // Copy non-@Attribute user fields
    final reservedFields = {
      ..._reservedFieldNames,
      ...attributes.keys,
      ...attributes.keys.map((k) => '_$k'),
    };
    ...
    for (final field in classElement.fields) {
      ...
      if (fieldSource != null && fieldSource.isNotEmpty) {
        ...
        // Fallback: reconstruct the field declaration from element metadata
        final typeStr = field.type.getDisplayString();
        final prefix =
            '${field.isLate ? 'late ' : ''}${field.isFinal ? 'final ' : ''}';

        final defaultValue = _getFieldInitializer(field);
        if (defaultValue != null && defaultValue.isNotEmpty) {
          buffer.writeln('  $prefix$typeStr $fieldName = $defaultValue;');
        } else {
          buffer.writeln(
            '  ${field.isLate ? 'late ' : ''}$typeStr $fieldName;',
          );
        }
      }
    }
      ...
      for (final param in constructor.formalParameters) {
        if (param.isNamed) {
          ...
          if (attributes.containsKey(paramName)) {
            // @Attribute field: assign to private backing field
            buffer.writeln('    _$paramName = $paramName;');
          } else {
            // Non-@Attribute field: assign directly if field exists
            final hasField = classElement.fields.any(
              (f) => f.name == paramName && !f.isStatic,
            );
            if (hasField && !_reservedFieldNames.contains(paramName)) {
              buffer.writeln('    this.$paramName = $paramName;');
            }
          }
        }
      }
  ...
  void _copyUserMethods(
    ...
    // Copy other non-static methods (excluding reserved/generated ones)
    ...
    // Copy user-defined getters (excluding reserved/generated ones)
    final reservedGetters = {
      ..._reservedGetterNames,
      ...attributes.keys, // @Attribute fields have generated getters
    };

    for (final getter in classElement.getters) {
      // Only copy explicit getter declarations, not field-synthesized ones
      if (!getter.isOriginDeclaration || getter.isStatic) continue;
    ...
    // Copy user-defined setters (excluding reserved/generated ones)
    final reservedSetters = <String>{
      ...attributes.keys, // @Attribute fields have generated setters
    };

    for (final setter in classElement.setters) {
      // Only copy explicit setter declarations, not field-synthesized ones
      if (!setter.isOriginDeclaration || setter.isStatic) continue;

      final rawName = setter.name;
      if (rawName == null) continue;
      // Dart analyzer appends '=' to setter names
  ...
  String? _getSetterSource(
    ...
    // Strip trailing '=' that the analyzer appends to setter names
    ...
    try {
      ...
      final pattern = RegExp(
        r'\bset\s+' + RegExp.escape(cleanName) + r'\s*\(',
        multiLine: true,
      );
      ...
      for (final match in matches) {
        // Go backwards to find start of declaration
        int start = match.start - 1;
        while (start > 0 && contents[start] != '}' && contents[start] != ';') {
          start--;
        }
        ...
        // Find the closing paren of parameter list
        int pos = match.end;
        int parenCount = 1;
        while (pos < contents.length && parenCount > 0) {
          if (contents[pos] == '(') parenCount++;
          if (contents[pos] == ')') parenCount--;
          pos++;
        }

        // Now find body - either => or {
        while (pos < contents.length &&
            contents[pos] != '=' &&
            contents[pos] != '{') {
          pos++;
      ...
      print('Failed to extract setter source for $cleanName: $e');
    }
  ...
  String? _getFieldSource(FieldElement field, String sourceFilePath) {
    ...
    try {
      ...
      for (final match in matches) {
        ...
        int end = match.end - 1; // back up from the ; or =
        if (contents[end] == ';') {
          end++; // include the semicolon
        } else {
          // It's '=' - find the matching semicolon (handling nested expressions)
          end = match.end;
          int braceCount = 0;
          int parenCount = 0;
          int bracketCount = 0;
      ...
      print('Failed to extract field source for $fieldName: $e');
    }

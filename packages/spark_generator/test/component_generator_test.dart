...
void main() {
  group('ComponentGenerator', () {
    ...
    test(
      'does not extract method calls inside other methods as method definitions',
      skip: 'Build test framework limitations prevent file-based testing',
      () async {
        // This test uses a real file fixture to test method extraction
        const fixturePath = 'lib/contact_form_test_base.dart';
        final fixtureContent = File(
          'test/fixtures/contact_form_base.dart',
        ).readAsStringSync();
        ...
        await resolveSources(
          ...
          ''',
            ...
            export 'src/component/spark_component.dart';
          ''',
            // Include the fixture file content in in-memory sources
            'a|$fixturePath': fixtureContent,
          },
          (resolver) async {
            final libraryElement = await resolver.libraryFor(
              AssetId('a', fixturePath),
            );

            final contactFormClass = libraryElement.children
                .whereType<ClassElement>()
                .firstWhere((e) => e.name == 'ContactForm');

            final annotations = contactFormClass.metadata.annotations;
            final annotation = annotations.firstWhere((a) {
              final element = a.element;
              final enclosing = element?.enclosingElement;
              return enclosing?.name == 'Component';
            ...
            final generator = ComponentGenerator();
            // Important: Pass the real file path to SimpleBuildStep
            // This enables _getMethodSource to read from the actual file
            final output = generator.generateForAnnotatedElement(
              contactFormClass,
              constantReader,
              SimpleBuildStep(AssetId('a', fixturePath)),
            );

            // Should copy _handleSubmit method exactly once
            final handleSubmitMatches = RegExp(
              r'void _handleSubmit\(\)',
            ).allMatches(output);
            expect(
              handleSubmitMatches.length,
              equals(1),
              reason:
                  '_handleSubmit should appear exactly once as a method definition',
            );

            // Should copy the render method as build
            expect(output, contains('Element build()'));
            expect(output, contains("onClick: (_) => _handleSubmit()"));

            // Should also copy the anotherHelper method
            expect(output, contains('void anotherHelper()'));

            // The method body should contain the call to _handleSubmit
            final anotherHelperMatch = RegExp(
              r'void anotherHelper\(\)\s*\{[^}]*\}',
              multiLine: true,
            ).firstMatch(output);
            expect(anotherHelperMatch, isNotNull);
            if (anotherHelperMatch != null) {
              final methodBody = output.substring(
                anotherHelperMatch.start,
                anotherHelperMatch.end,
              );
              expect(methodBody, contains('_handleSubmit()'));
            }

            // Should NOT contain partial code fragments
            // The bug would cause fragments to appear as standalone text
            final onClickPattern = RegExp(
              r'^\s*onClick:\s*\(_\)\s*=>\s*_handleSubmit\(\)',
              multiLine: true,
            );
            final onClickMatches = onClickPattern.allMatches(output);

            // onClick should only appear once, inside the build method
            expect(
              onClickMatches.length,
              equals(1),
              reason: 'onClick handler should only appear inside build method',
            );
          },
        );
      },

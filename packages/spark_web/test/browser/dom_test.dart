...
void main() {
  group('Browser DOM Wrapping', () {
    test('wrapNode handles various types', () {
      expect(
        wrapNode(web.document.createElement('div')),
        isA<spark.HTMLDivElement>(),
      );
      expect(
        wrapNode(web.document.createElement('span')),
        isA<spark.HTMLSpanElement>(),
      );
      expect(
        wrapNode(web.document.createElement('input')),
        isA<spark.HTMLInputElement>(),
      );
      expect(
        wrapNode(web.document.createElement('button')),
        isA<spark.HTMLButtonElement>(),
      );
      expect(
        wrapNode(web.document.createElement('textarea')),
        isA<spark.HTMLTextAreaElement>(),
      );
      expect(
        wrapNode(web.document.createElement('select')),
        isA<spark.HTMLSelectElement>(),
      );
      expect(
        wrapNode(web.document.createElement('option')),
        isA<spark.HTMLOptionElement>(),
      );
      expect(
        wrapNode(web.document.createElement('a')),
        isA<spark.HTMLAnchorElement>(),
      );
      expect(
        wrapNode(web.document.createElement('img')),
        isA<spark.HTMLImageElement>(),
      );
      expect(
        wrapNode(web.document.createElement('form')),
        isA<spark.HTMLFormElement>(),
      );
      expect(
        wrapNode(web.document.createElement('label')),
        isA<spark.HTMLLabelElement>(),
      );
      expect(
        wrapNode(web.document.createElement('template')),
        isA<spark.HTMLTemplateElement>(),
      );
      expect(
        wrapNode(web.document.createElement('canvas')),
        isA<spark.HTMLCanvasElement>(),
      );
      expect(
        wrapNode(web.document.createElement('video')),
        isA<spark.HTMLVideoElement>(),
      );
      expect(
        wrapNode(web.document.createElement('audio')),
        isA<spark.HTMLAudioElement>(),
      );
      expect(
        wrapNode(web.document.createElement('dialog')),
        isA<spark.HTMLDialogElement>(),
      );
      expect(
        wrapNode(web.document.createElement('details')),
        isA<spark.HTMLDetailsElement>(),
      );
      expect(
        wrapNode(web.document.createElement('slot')),
        isA<spark.HTMLSlotElement>(),
      );
      expect(
        wrapNode(web.document.createElement('iframe')),
        isA<spark.HTMLIFrameElement>(),
      );
      expect(
        wrapNode(web.document.createElement('table')),
        isA<spark.HTMLTableElement>(),
      );
      expect(
        wrapNode(web.document.createElement('progress')),
        isA<spark.HTMLProgressElement>(),
      );
      expect(
        wrapNode(web.document.createElement('meter')),
        isA<spark.HTMLMeterElement>(),
      );
      expect(
        wrapNode(web.document.createElement('output')),
        isA<spark.HTMLOutputElement>(),
      );

      final table = web.document.createElement('table') as web.HTMLTableElement;
      expect(
        wrapNode(table.createTBody()),
        isA<spark.HTMLTableSectionElement>(),
      );
      expect(wrapNode(table.insertRow()), isA<spark.HTMLTableRowElement>());

      expect(
        wrapNode(web.document.createElement('ol')),
        isA<spark.HTMLOListElement>(),
      );
      expect(
        wrapNode(web.document.createElement('li')),
        isA<spark.HTMLLIElement>(),
      );
      expect(
        wrapNode(web.document.createElement('p')),
        isA<spark.HTMLParagraphElement>(),
      );
      ...
      expect(
        wrapNode(web.document.createDocumentFragment()),
        isA<spark.DocumentFragment>(),
      );
    ...
    test('wrapEvent handles various types', () {
      ...
      expect(
        wrapEvent(web.KeyboardEvent('keydown')),
        isA<spark.KeyboardEvent>(),
      );
      ...
      expect(
        wrapEvent(web.PointerEvent('pointerdown')),
        isA<spark.PointerEvent>(),
      );
      expect(
        wrapEvent(web.AnimationEvent('animationend')),
        isA<spark.AnimationEvent>(),
      );
      expect(
        wrapEvent(web.TransitionEvent('transitionend')),
        isA<spark.TransitionEvent>(),
      );
  ...
  group('BrowserHTMLElement', () {
    ...
    test('BrowserHTMLOutputElement', () {
      final el =
          spark.document.createElement('output') as spark.HTMLOutputElement;
      el.value = 'foo';
      expect(el.value, 'foo');
      el.defaultValue = 'bar';
    ...
    test('BrowserHTMLProgressElement', () {
      final el =
          spark.document.createElement('progress') as spark.HTMLProgressElement;
      el.value = 0.5;
      expect(el.value, 0.5);
      el.max = 1.0;
      expect(el.max, 1.0);
    ...
    test('BrowserHTMLMeterElement', () {
      final el =
          spark.document.createElement('meter') as spark.HTMLMeterElement;
      el.min = 0;
      expect(el.min, 0);
      el.max = 100;
      expect(el.max, 100);
